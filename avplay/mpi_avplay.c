...
typedef struct
{
    ...
    HI_UNF_AVPLAY_STATUS_E          AudStatus;
    HI_UNF_AVPLAY_STATUS_E          VidStatus;
...
HI_VOID AVPLAY_ProcAdecToAo(AVPLAY_S *pAvplay)
{
    ...
    if (HI_UNF_AVPLAY_STATUS_PAUSE == pAvplay->CurStatus || HI_UNF_AVPLAY_STATUS_PAUSE == pAvplay->AudStatus)
    {
        return;
    }
    ...
    if (pAvplay->AvplayProcDataFlag[AVPLAY_PROC_ADEC_AO])
    {
        if (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus || HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->AudStatus)
        {
...
HI_VOID AVPLAY_ProcVidFrc(AVPLAY_S *pAvplay)
{
    ...
    if (HI_TRUE == pAvplay->bFrcEnable && (HI_UNF_AVPLAY_STATUS_TPLAY != pAvplay->VidStatus))
    {
...
HI_VOID AVPLAY_ProcVidSync(AVPLAY_S *pAvplay)
{
    ...
    for (i=0; i<pAvplay->CurFrmPack.u32FrmNum; i++)
    {
        ...
        if (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus || HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->VidStatus)
        {
            pAvplay->CurFrmPack.stFrame[i].stFrameVideo.enTBAdjust = HI_DRV_VIDEO_TB_PLAY;
        }
    }
...
HI_S32 AVPLAY_AllocHDRChn(AVPLAY_S *pAvplay, AVPLAY_HDR_CHAN_E HDRChn)
{
    ...
    if (HDRChn & ((HI_U32)AVPLAY_HDR_CHAN_EL))
    {
...
#ifdef HI_TEE_SUPPORT
        Ret = HI_MPI_VDEC_SetTVP(VdecHandle, &(pAvplay->TVPAttr));
        if (HI_SUCCESS != Ret)
        {
        ...
        Ret = HI_MPI_VDEC_ChanBufferInit(VdecHandle, HI_INVALID_HANDLE, &stVdecBufAttr);
#else
        Ret = HI_MPI_VDEC_ChanBufferInit(VdecHandle, pAvplay->AvplayAttr.stStreamAttr.u32VidBufSize, HI_INVALID_HANDLE);
#endif
        if (Ret != HI_SUCCESS)
        {
...
HI_VOID AVPLAY_ProcVdecToVo(AVPLAY_S *pAvplay)
{
    ...
    if (HI_UNF_AVPLAY_STATUS_PAUSE == pAvplay->CurStatus || HI_UNF_AVPLAY_STATUS_PAUSE == pAvplay->VidStatus)
    {
        return;
    }
...
HI_S32 AVPLAY_SetStreamMode(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_ATTR_S *pAvplayAttr)
{
    ...
    if (pAvplay->hVdec != HI_INVALID_HANDLE)
    {
            ...
            || (CurVidBufSize != pAvplayAttr->stStreamAttr.u32VidBufSize) )
        {
            ...
            #ifdef HI_TEE_SUPPORT
                VdecBufAttr.u32BufSize = 0;
                VdecBufAttr.bTvp = pAvplay->TVPAttr.bEnable;
                Ret = HI_MPI_VDEC_ChanBufferInit(pAvplay->hVdec, pAvplay->hDmxVid, &VdecBufAttr);
            #else
                Ret = HI_MPI_VDEC_ChanBufferInit(pAvplay->hVdec, 0, pAvplay->hDmxVid);
            #endif
                if (Ret != HI_SUCCESS)
            ...
            {
            #ifdef HI_TEE_SUPPORT
                VdecBufAttr.u32BufSize = pAvplayAttr->stStreamAttr.u32VidBufSize;
                VdecBufAttr.bTvp = pAvplay->TVPAttr.bEnable;
                Ret = HI_MPI_VDEC_ChanBufferInit(pAvplay->hVdec, HI_INVALID_HANDLE, &VdecBufAttr);
            #else
                Ret = HI_MPI_VDEC_ChanBufferInit(pAvplay->hVdec, pAvplayAttr->stStreamAttr.u32VidBufSize, HI_INVALID_HANDLE);
            #endif
                if (Ret != HI_SUCCESS)
                {
                    HI_ERR_AVPLAY("HI_MPI_VDEC_ChanBufferInit failed 0x%x\n",Ret);
...
static HI_S32 AVPLAY_StopAudChn(const AVPLAY_S *pAvplay)
{
    HI_S32         Ret;
    HI_U32         i;
...
HI_VOID AVPLAY_Play(AVPLAY_S *pAvplay)
{
    if (pAvplay->CurStatus != HI_UNF_AVPLAY_STATUS_PLAY)
    {
        ...
        pAvplay->AudStatus = pAvplay->CurStatus;
        pAvplay->VidStatus = pAvplay->CurStatus;
    }
...
HI_S32 HI_MPI_AVPLAY_Create(const HI_UNF_AVPLAY_ATTR_S *pstAvAttr, HI_HANDLE *phAvplay)
{
    ...
    pAvplay->AudStatus = HI_UNF_AVPLAY_STATUS_STOP;
    pAvplay->VidStatus = HI_UNF_AVPLAY_STATUS_STOP;
...
HI_S32 HI_MPI_AVPLAY_ChangeStatus(HI_HANDLE hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_E enChn, HI_UNF_AVPLAY_STATUS_E enStatus)
{
    AVPLAY_S   *pAvplay;
    HI_U32      Id = GET_AVPLAY_ID(hAvplay);
    ...
    if (HI_UNF_AVPLAY_MEDIA_CHAN_AUD == enChn)
        pAvplay->AudStatus = enStatus;
    else if (HI_UNF_AVPLAY_MEDIA_CHAN_VID == enChn)
        pAvplay->VidStatus = enStatus;
...
HI_S32 HI_MPI_AVPLAY_ClearBuffer(HI_HANDLE hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_E enChn)
{
    AVPLAY_S   *pAvplay;
    HI_U32      Id = GET_AVPLAY_ID(hAvplay);
